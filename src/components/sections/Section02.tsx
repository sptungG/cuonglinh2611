...
const Section02 = ({ userData }: TSection02Props) => {
  return (
    <section className="relative flex max-w-[100dvw] flex-col items-center justify-center overflow-hidden max-sm:py-20 sm:pb-20">
      ...
      <div className="mb-4 text-center text-xl uppercase text-amber-500">{userData?.partyName === "NhaGai" ? "Tại nhà gái" : "Tại nhà trai"}</div>
      <div className="mb-2 flex items-center gap-1 text-base leading-[1.2] sm:text-lg">
        <span>{userData?.partyName === "NhaGai" ? "14/305 Phúc Tân, Hoàn Kiếm, Hà Nội" : "Đội 5, Phú Thịnh, Kim Động, Hưng Yên"}</span>
      </div>
      <MotionEffect slide={{ direction: "left" }} inView inViewOnce={false} className="sm:px-10 sm:py-5">
